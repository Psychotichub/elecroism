  ...
  return (
    <>
      <Rect
        ...
        {...strokeProps(SymbolMetrics.stroke)}
...
export const TerminalPocket: React.FC<TerminalPocketProps> = ({
  ...
  const { strokeProps, fontSize } = useSymbolStrokes();
  return (
    <>
      <Rect
        x={x - width / 2}
        ...
        {...strokeProps(SymbolMetrics.detailStroke)}
        cornerRadius={1.4}
        listening={false}
      />
      <Circle
    ...
    <>
      ...
      {text && (
        <Text
          ...
          width={width - 4}
          align="center"
          fontSize={fontSize(Math.min(7, Math.max(5, height - 4)))}
          fontStyle="bold"
...
export const PhaseTag: React.FC<PhaseTagProps> = ({
  ...
  const { fontSize } = useSymbolStrokes();
  return (
    <Text
      text={text}
      x={x - 6}
